...
export default function Sidebar({ userName, userRole, open, onClose }: SidebarProps) {
      ...
      <div className="px-5 py-5 border-b border-white/10 flex items-center justify-between">
        <div className="flex items-center gap-3">
          <div className="w-9 h-9 rounded-lg flex items-center justify-center text-sm font-bold text-white border border-white/20"
            style={{ backgroundColor: 'var(--navy-light)' }}>LB</div>
          <div>
        ...
        {nav.map((item) => {
                  ...
                  {item.children.map(child => {
                    ...
                    return (
                      <Link key={child.href} href={child.href} onClick={onClose}
                        className={`block px-3 py-1.5 rounded-lg text-sm transition ${active ? 'text-white font-medium' : 'text-white/50 hover:text-white/80'}`}
          ...
          return (
            <Link key={item.href} href={item.href} onClick={onClose}
              className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium transition ${active ? 'text-white' : 'text-white/50 hover:text-white/80'}`}
  ...
  return (
    <>
      <div className="hidden md:block w-60 flex-shrink-0">{content}</div>
      {open && (
        <>
